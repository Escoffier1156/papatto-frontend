...
import { Heart, MapPin, Sparkles, Send, X, ShieldCheck, Zap, MessageCircle } from 'lucide-react';
...
export const UserProfileCard: React.FC<UserProfileCardProps> = ({ user, onSendLike, onStartChat }) => {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    if (onStartChat) {
      onStartChat(user, chatInput.trim());
    }
        ...
        <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
          ...
          <div className="pt-2 flex items-center justify-between border-t border-slate-100 dark:border-slate-800">
            <button
              ...
              className="flex items-center gap-1 text-xs text-pink-600 font-bold hover:underline"
            >
                ...
                {messageSent ? (
                  ...
                  <form onSubmit={handleSendMessage} className="space-y-2">
                    <div className="relative">
                      <input
                        type="text"
                        placeholder={`${user.nickname}さんに12時間消滅メッセージを送る...`}
                        value={chatInput}
